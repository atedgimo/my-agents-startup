...
The main.py imports ghost_ai and ghost_visuals from src.backend, but the repo structure is src/backend/ not a package with __init__.py, so the import should be relative or adjusted.

The error says "No module named 'src'", which means the import style is incorrect for the repo layout.

I see main.py imports like:

from src.backend.ghost_visuals import GhostManager, GhostIdentity, GhostState

But the repo structure is src/backend/ghost_visuals.py, not a package named src.

To fix this, the import should be:

from ghost_visuals import GhostManager, GhostIdentity, GhostState

and similarly for ghost_ai if used.

I will fix the imports in src/backend/main.py to be relative imports or direct imports without src prefix, so the modules exist and the app can start.

Then I will move card 0050 to review.
